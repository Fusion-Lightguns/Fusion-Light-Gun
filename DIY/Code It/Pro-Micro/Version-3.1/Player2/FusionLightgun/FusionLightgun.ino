 ...
 /*!
   @file FusionLightgun.ino
   @brief Fusion Lightgun code orginally taken from Samco and Pwor7. Updated by Gonezo from Fusion Lightguns.
...
#include <FusionPositionEnhanced.h>
#include <FusionConst.h>
...
const LightgunButtons::Desc_t LightgunButtons::ButtonDesc[] = {
  {10, LightgunButtons::ReportType_Mouse, MOUSE_LEFT, 25, BTN_AG_MASK, "Trigger"},
  {9, LightgunButtons::ReportType_Keyboard, KEY_LEFT_SHIFT, 25, BTN_AG_MASK2, "A"},
  {8, LightgunButtons::ReportType_Keyboard, KEY_LEFT_CTRL, 25, BTN_AG_MASK2, "B"},
  {7, LightgunButtons::ReportType_Keyboard, KEY_2, 20, BTN_AG_MASK2, "Start"},
  {6, LightgunButtons::ReportType_Keyboard, KEY_6, 25, BTN_AG_MASK2, "Select"},
  {14, LightgunButtons::ReportType_Mouse, MOUSE_BUTTON5, 20, BTN_AG_MASK2, "Calibrate"},
  {15, LightgunButtons::ReportType_Mouse, MOUSE_MIDDLE, 20, BTN_AG_MASK2, "Bomb"},
...
// if you have original Fusion calibration values, multiply by 4 for the center position and
// scale is multiplied by 1000 and stored as an unsigned integer, see FusionPreferences::Calibration_t
FusionPreferences::ProfileData_t profileData[ProfileCount] = {
...
// Fusion positioning
FusionPositionEnhanced myFusion;
...
int nvPrefsError = FusionPreferences::Error_NoStorage;

// preferences instance
FusionPreferences elitePreferences;

// number of times the IR camera will update per second
constexpr unsigned int IRCamUpdateRate = 209;

#ifdef FUSION_NO_HW_TIMER
...
#define FUSION_NO_HW_TIMER_UPDATE() NoHardwareTimerCamTickMillis()
//define FUSION_NO_HW_TIMER_UPDATE() NoHardwareTimerCamTickMicros()

#else
#define FUSION_NO_HW_TIMER_UPDATE()
// timer will set this to 1 when the IR position can update
volatile unsigned int irPosUpdateTick = 0;
#endif // FUSION_NO_HW_TIMER
...
#if defined(FUSION_SAMD21) || defined(FUSION_SAMD51)
...
void check_trigger()
{
  if (buttons.pressedReleased == BtnMask_Trigger) stop_motor();
  if (buttons.debounced == BtnMask_Trigger) recoil();
}
...
void ExecRunMode()
{
  ...
  for (;;) {
    ...
    if (irPosUpdateTick) {
      ...
      int halfHscale = (int)(myFusion.h() * xScale + 0.5f) / 2;
      moveXAxis = map(finalX, xCenter + halfHscale, xCenter - halfHscale, 0, MouseMaxX);
      halfHscale = (int)(myFusion.h() * yScale + 0.5f) / 2;
...
// from Fusion_4IR_Test_BETA sketch
// for use with the Fusion_4IR_Processing_Sketch_BETA Processing sketch
  ...
  for (;;) {
    ...
    if (irPosUpdateTick) {
      ...
      if (error == DFRobotIRPositionEx::Error_Success) {
        myFusion.begin(dfrIRPos.xPositions(), dfrIRPos.yPositions(), dfrIRPos.seen(), MouseMaxX / 2, MouseMaxY / 2);
        UpdateLastSeen();
        for (int i = 0; i < 4; i++) {
          Serial.print(map(myFusion.testX(i), 0, MouseMaxX, CamMaxX, 0) + processingOffset);
          Serial.print(",");
          Serial.print(map(myFusion.testY(i), 0, MouseMaxY, CamMaxY, 0) + processingOffset);
          Serial.print(",");
        }
        Serial.print(map(myFusion.x(), 0, MouseMaxX, CamMaxX, 0) + processingOffset);
        Serial.print(",");
        Serial.print(map(myFusion.y(), 0, MouseMaxY, CamMaxY, 0) + processingOffset);
        Serial.print(",");
        Serial.print(map(myFusion.testMedianX(), 0, MouseMaxX, CamMaxX, 0) + processingOffset);
        Serial.print(",");
        Serial.println(map(myFusion.testMedianY(), 0, MouseMaxY, CamMaxY, 0) + processingOffset);
...
{
  if (GetPositionIfReady()) {
    int halfH = (int)(myFusion.h() * yScale + 0.5f) / 2;
...
{
  if (GetPositionIfReady()) {
    int halfH = (int)(myFusion.h() * xScale + 0.5f) / 2;
...
void GetPosition()
{
  int error = dfrIRPos.basicAtomic(DFRobotIRPositionEx::Retry_2);
  if (error == DFRobotIRPositionEx::Error_Success) {
    myFusion.begin(dfrIRPos.xPositions(), dfrIRPos.yPositions(), dfrIRPos.seen(), xCenter, yCenter);
#ifdef EXTRA_POS_GLITCH_FILTER
    if ((abs(myFusion.X() - finalX) > BadMoveThreshold || abs(myFusion.Y() - finalY) > BadMoveThreshold) && badFinalTick < BadMoveCountThreshold) {
      ++badFinalTick;
    } else {
      if (badFinalTick) {
        badFinalCount++;
...
void UpdateLastSeen() {
  if (lastSeen != myFusion.seen()) {
    if (!lastSeen && myFusion.seen()) {
      LedOff();
    } else if (lastSeen && !myFusion.seen()) {
      SetLedPackedColor(IRSeen0Color);
    }
    lastSeen = myFusion.seen();
  }
}
...
void PrintPreferences()
{
  ...
  Serial.println(profileDesc[FusionPreferences::preferences.profile].profileLabel);

  Serial.println("Profiles:");
  for (unsigned int i = 0; i < FusionPreferences::preferences.profileCount; ++i) {
...
{
#ifdef FUSION_FLASH_ENABLE
  unsigned int required = FusionPreferences::Size();
  ...
  Serial.print((unsigned int)sizeof(FusionPreferences::ProfileData_t));
...
{
  if (nvPrefsError != FusionPreferences::Error_Success) {
    Serial.print(NVRAMlabel);
    Serial.print(" error: ");
#ifdef FUSION_FLASH_ENABLE
    Serial.println(FusionPreferences::ErrorCodeToString(nvPrefsError));
#else
...
#ifdef FUSION_FLASH_ENABLE
  nvPrefsError = FusionPreferences::Load(flash);
#else
...
void VerifyPreferences()
{
  ...
  if (FusionPreferences::preferences.profile >= ProfileCount) {
    FusionPreferences::preferences.profile = (uint8_t)selectedProfile;
  }
}
...
void ApplyInitialPrefs()
{
  ...
  if (FusionPreferences::preferences.profile < ProfileCount) {
    ...
    selectedProfile = FusionPreferences::preferences.profile;
...
#ifdef FUSION_FLASH_ENABLE
  nvPrefsError = FusionPreferences::Save(flash);
#else
  nvPrefsError = FusionPreferences::Save();
#endif // FUSION_FLASH_ENABLE
  if (nvPrefsError == FusionPreferences::Error_Success) {
